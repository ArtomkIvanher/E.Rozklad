...
export default function SubjectsManager({ subjects, setSubjects, onAddSubject }) {
  const [newSubject, setNewSubject] = React.useState({ name: "", teacher: "", zoom_link: "" });
    ...
    <div>
      ...
      {subjects.map((subject) => (
        <div key={subject.id}>
          ...
          <button onClick={() => handleRemoveSubject(subject.id)}>Видалити</button>
        </div>
      ))}
    </div>
  );
